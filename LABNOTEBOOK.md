...
- Five continuous gene-summed ABC annotations are rebuilt in the exact 1000G BIM row order. Their LD scores are computed per chromosome with the official `ldsc.py --l2` implementation, a 1 cM LD window, MAF at least 0.05 for annotation totals, and the BaselineLD regression-SNP rows as the output set.
