...
- Selected ancestry-matched public summary statistics for five additional traits: no-proxy European PD from Nalls et al. 2019 (`GCST009324`; 15,056 cases and 12,637 controls), PGC4 European bipolar disorder excluding 23andMe (O'Connell et al. 2025), PGC MDD2025 European excluding 23andMe (Adams et al. 2025), Project MinE European ALS (`GCST90027164`; van Rheenen et al. 2021), and the European genomic-factor anxiety GWAS from Friligkou et al. 2024. The newer 2024 PD release was not selected because only its multi-ancestry meta-analysis is publicly downloadable, which is mismatched to the European UKBB LD reference. PD excludes proxy cases, matching the no-proxy AD design.
...
- Download and normalization job `19304890` feeds trait-specific official BaselineLD H3K27ac S-LDSC jobs, followed by ten-fold corrected signed gene-summed COMPASS, canonical refits, and 25%/50% contribution-weighted GSEA. The complete machine-readable job table is in the analysis logs as `neuropsychiatric-pipeline-jobs-20260722.tsv`. S-LDSC build jobs are `19304891` (PD), `19304910` (bipolar), `19304929` (MDD), `19304948` (ALS), and `19304967` (anxiety); downstream jobs are dependency-gated on successful normalization and context-profile extraction.
