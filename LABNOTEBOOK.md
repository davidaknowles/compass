...
- A follow-up `lr=3e-7` profile accelerated AD `lambda=10` by another factor of about three and gave provisional held-out MSE `37.8414`; microglia comprised 52.0% of aggregate mass. However, the next `lambda=3` objective increased after the first update, demonstrating instability at weaker penalties. Production therefore retains the validated `lr=1e-7` across the complete grid.
