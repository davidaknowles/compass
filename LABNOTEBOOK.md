...
- Preparation, all BaselineLD-adjusted S-LDSC fits, all 50 CV folds, merges, and full refits completed successfully. PD selected `lambda=100` and retained gene-specific deviations; its fitted context fractions were OPC 50.5%, microglia 34.1%, and astrocyte 15.4%, with negligible mass in the other contexts. Anxiety selected `lambda=30` but its full refit had zero gene-specific deviations. Bipolar, MDD, and ALS selected the maximum tested `lambda=1e6` and also had zero deviations, so their fitted mass is entirely the frozen nonnegative S-LDSC context profile plus the residual term.
- Initial GSEA jobs failed because the submission wrapper added a nonexistent `.gwas` suffix to the annotation cache key. The lookup was corrected and replacement jobs `19312131`--`19312140` all completed. PD yielded 12 and 21 g:SCS-significant terms at the 25% and 50% cumulative deviation-contribution thresholds, including global synaptic-vesicle endocytosis and recycling terms. The other traits correctly produced empty deviation-based queries because their fitted deviations were exactly zero.
...
## 2026-07-22 additional neurological and psychiatric GWAS

- Selected ancestry-matched public summary statistics for five additional traits: the no-proxy clinical European PD meta-analysis from Leonard et al. 2025 (34,933 cases and 31,009 controls), PGC4 European bipolar disorder excluding 23andMe (O'Connell et al. 2025), PGC MDD2025 European excluding 23andMe (Adams et al. 2025), Project MinE European ALS (`GCST90027164`; van Rheenen et al. 2021), and the European genomic-factor anxiety GWAS from Friligkou et al. 2024. The PD archive is hosted by the Neurodegenerative Disease Knowledge Portal and includes separate ancestry and ascertainment analyses; we use `GP2_ALL_EUR_CLINICAL_ONLY_HG38_12162024.txt.gz` to exclude proxy and biobank-defined cases. PD coordinates are explicitly lifted from GRCh38 to hg19 before harmonization.
- Added an atomic public-GWAS downloader and streaming normalizer. Bipolar and MDD use released variant-level effective sample sizes (`Neff = 2 * Neff_half` where needed), ALS uses released `N_effective`, PD uses case-control effective `N=27,481.7`, and anxiety uses the published European analysis size `N=1,096,458` because the genomic-factor file omits variant-level sample size. Every normalized row carries known `N`; all selected files are already GRCh37/hg19.
- Download and normalization job `19304890` prepared bipolar disorder, MDD, ALS, anxiety, and an initially selected 2019 PD release. PD jobs `19304891`-`19304909` were cancelled before fitting after identifying the larger Leonard et al. 2025 European release; the replacement PD job IDs are recorded below. Trait-specific official BaselineLD H3K27ac S-LDSC jobs feed ten-fold corrected signed gene-summed COMPASS, canonical refits, and 25%/50% contribution-weighted GSEA. The original machine-readable job table is in the analysis logs as `neuropsychiatric-pipeline-jobs-20260722.tsv`.
- The 2025 PD replacement uses preparation job `19305442`, S-LDSC build/LD-score/fit/extract jobs `19305443`-`19305446`, COMPASS fold jobs `19305447`-`19305456`, merge/refit jobs `19305458`-`19305459`, and GSEA jobs `19305460`-`19305461`. The machine-readable replacement table is `pd2025-pipeline-jobs-20260722.tsv` in the analysis logs.
- Initial PD S-LDSC fit `19305445` found zero overlapping SNP IDs because the GP2 coordinate-based IDs remained in GRCh38 notation after coordinate liftOver. The reusable S-LDSC exporter now assigns 1000 Genomes rsIDs using unique lifted chromosome-position matches. The corrected build/LD-score/fit/extract jobs are `19305617`-`19305620`, COMPASS folds are `19305621`-`19305630`, and merge/refit/GSEA jobs are `19305632`-`19305635`; the replacement job table is `pd2025-pipeline-jobs-20260722-r2.tsv`.
- Corrected S-LDSC completed with 1,175,866 regression SNPs and observed-scale heritability 0.2566 (SE 0.0205). The first corrected COMPASS submission exhausted its 96 GB host-memory allocation while independently constructing the same genome-wide LD cache in all ten folds (peak RSS 83-98 GB before setup completed). Subsequent genome-wide fold jobs request 192 GB, use four LD loader threads, and serialize the first fold as the shared cache builder before releasing the remaining folds.
- The memory-corrected PD COMPASS submission uses fold 0 cache-builder job `19306473`, dependent fold jobs `19306474`-`19306482`, merge/refit jobs `19306484`-`19306485`, and GSEA jobs `19306486`-`19306487`. The job table is `pd2025-compass-jobs-20260722-r3.tsv` in the analysis logs.
...
- Added isolated hierarchical CV fold execution and a checkpoint merger so remaining folds can run concurrently without concurrent writes to one checkpoint. Each shard retains the original LD-component labels and trains against all non-held-out rows; only execution scheduling is partitioned. The merged checkpoint validates the lambda/profile/model contract, requires every fold to be complete and finite, and supports an independent full-data refit for comparison with the original sequential run.
- The AD 2026 hierarchical fit completed all ten folds. The context-only plateau had mean held-out MSE `15.9205032746`; the nominally selected `lambda=100000` differed by only `1.5e-9`, while `lambda=100` scored `16.0048359461` and weaker penalties were worse. The final deviation matrix is exactly zero. The frozen global layer assigns 82.52% of biological context contribution to microglia, 10.37% to oligodendrocytes, and 7.11% to neurons, so it passes the expected AD ranking while providing no independently learned gene-deviation signal.
- Submitted isolated SCZ folds 6--9 as jobs `19290710`--`19290713`, excluding the node used by the sequential fit. Merge job `19290960` combines completed shard rows with sequential folds 0--5 into a separate checkpoint; independent refit `19290961` and summary `19290962` follow. The original SCZ chain remains active and untouched.
- The merged SCZ checkpoint passed its completeness and finite-score checks. Across all ten folds, `lambda=100` had mean MSE `12.09113058`, compared with `12.53763068` for the context-only plateau, a 3.56% improvement. `lambda=30` and `10` scored `20.22142493` and `20.22134021`, respectively. Cross-node duplication of fold 6 agreed within `9.5e-5` at the context-only plateau and `6.8e-5` at `lambda=100`; weak-penalty scores differed by 2.7% but remained far outside the selected region.
- The independent SCZ full-data refit completed at `lambda=100`. Neurons contribute 56.35% of total modeled mass and 68.63% after excluding the gene-shared intercept; astrocytes contribute 10.58%, oligodendrocytes 10.63%, and microglia 4.54% of total mass. The cross-trait audit passes AD=microglia and SCZ=neuron. The SCZ deviation matrix has 6,154 positive genes and is numerically rank one (`3.45e-5` leading singular value; remaining values about `3e-13` to `4e-13`), so the predictive gene deviation is shared across contexts rather than cell-type specific.
- Raw SCZ deviation coefficients contain a 397-gene tie at the maximum, making an alphabetically tie-broken top-100 analysis invalid. Post-fit rankings now optionally multiply each gene-context coefficient by its annotation-column mass and rank global genes by summed fitted deviation contribution. This produces 5,482 unique positive contribution scores among 6,154 positive genes. Leading genes include `KIF21B`, `TRIM9`, `RASA3`, `PPP1R16B`, `ZBTB16`, `PTN`, `TSNARE1`, `KCNB1`, `IGSF9B`, and `CDK5R1`.
- The initial top-100 ordered enrichment was only a legacy post-fit convention. The selected SCZ deviation is not elementwise sparse: 6,154 of 16,283 genes (37.8%) have `D > 0`, and every positive gene is positive in all five contexts. Querying all positive genes would retain thousands of negligible effects. The primary ordered g:Profiler analysis instead uses the smallest leading set explaining 25% of fitted deviation contribution, with 50% as a sensitivity analysis and all assayed genes as background. The 25% sets contain 334 global genes and 206--232 context-specific genes; the corresponding 50% sets contain 940 and 648--699 genes. Top-100 files are retained only as readable summaries.
- The preceding SCZ deviation and enrichment are superseded as inferential results: the hierarchical optimizer constrained `D >= 0`, although the biological requirement is only `s q + D >= 0`. It also combined flat peak context annotations with gene-linked deviations, so it did not implement the exact `B = s q + D` decomposition in the manuscript. The corrected model uses gene-summed context annotations, signed deviations, and a shifted lower bound `D >= -s q`. Its proximal update uses Dykstra iterations to solve the nuclear-norm plus shifted-box proximal problem rather than applying a single post-SVD clamp. Synthetic noiseless testing recovers negative deviations to `1e-6` while retaining non-negative total coefficients. SCZ enrichment must be rerun after corrected CV and refitting.
- Corrected SCZ fold 0 job `19294297` validates real-data behavior at `lambda=100`: held-out MSE improves from `9.641692` in the context-only limit to `9.567555`; the saved deviation has 26,838 negative and 29,060 positive entries, while the minimum total coefficient `s q + D` is zero. Per-iteration runtime remains about six seconds because LD streaming, not the shifted proximal operator, dominates. Jobs `19294816`--`19294824` run folds 1--9 in isolated checkpoint namespaces. The shard merger can now construct a complete checkpoint without a sequential base, validating every model-contract field and fold before final refitting.
- The corrected signed gene-summed SCZ fit completed all ten folds and selected `lambda=100`, with mean held-out MSE `13.881955` versus `14.224715` for the context-only plateau (2.41% lower). The weaker values `lambda=30` and `10` scored `21.235829` and `21.236011`. In the full-data fit, neuron accounted for 50.22% of modeled contribution, the gene-shared intercept 21.46%, astrocyte 11.56%, oligodendrocyte 11.53%, and microglia 5.23%. The deviation matrix has 30,058 positive and 29,989 negative entries; every total coefficient satisfies `s q + D >= 0`, with a minimum of exactly zero. Two singular values are resolved (`3.24e-5` and `8.96e-6`), while the remaining three are below `1.4e-10`.
- Corrected ordered g:Profiler analyses rank genes by annotation mass times signed positive deviation contribution and use all 16,283 assayed genes as background. The 25% contribution sets contain 326 global genes and 196--245 context-specific genes; the 50% sets contain 921 and 613--716 genes. At 25%, the neuronal ranking is enriched for synapse organization and the microglial ranking for cytokine, interferon, and immune-receptor signaling. At 50%, neuronal and global rankings show stronger synapse organization, neuronal-system, trans-synaptic-signaling, neurogenesis, and 16p11.2 deletion terms; microglial immune enrichment persists. These are exploratory within-ranking g:SCS results rather than independent discoveries.
- Submitted a matched elementwise-nonnegative ablation using the same gene-summed annotations, frozen context profile, LD-component folds, lambda grid, optimizer, and fp32 representation. Fold jobs are `19299714`--`19299723`, merge job `19299729`, and final refit `19299730`. Only the deviation constraint differs, so its completed CV score can test whether allowing signed deviations improves prediction.
- The matched nonnegative ablation completed and also selected `lambda=100`. Its mean held-out MSE was `13.928728`, compared with `13.881955` for signed deviations, a 0.34% relative advantage for the signed model. The paired signed-minus-nonnegative fold difference was `-0.04677` (SE `0.10514`; paired $t$-test `p=0.667`; Wilcoxon `p=1.0`), so there is no evidence that signed deviations improve held-out prediction. The nonnegative full fit was rank one, with 31,235 positive entries and 50,180 zeros; its full-data objective was `9.7025`, compared with `9.2668` for the rank-two signed fit. The signed formulation is biologically less restrictive and fits the complete data better, but its additional flexibility is not supported by a resolved CV improvement in this analysis.
