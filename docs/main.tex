...
Unlike AD, the uncalibrated hierarchical COMPASS H3K27ac pilot independently ranked neurons first. The neuronal coefficient was $3.65\times10^{-7}$ ($z=2.46$), corresponding to an implied annotated $h^2$ of 0.216. Astrocytes were also positive ($z=2.34$), so the result had the expected direction but weaker specificity than S-LDSC. A gene-linked peak-to-expressed-TSS pilot gave the same ordering. In the corrected hierarchy, the global context design is the gene-wise sum of the same linked annotations used for the deviations, $D$ may be signed, and only the total coefficient is constrained: $s q + D\geq0$. Ten-fold LD-component cross-validation selected $\lambda=100$, with mean held-out MSE 13.8820 versus 14.2247 in the context-only limit, a 2.41\% improvement. The weaker values $\lambda=30$ and 10 both had MSE 21.236, separating the selected value from the overfit tail. In the full-data fit, neurons accounted for 50.2\% of total modeled contribution, the gene-shared intercept for 21.5\%, astrocytes for 11.6\%, oligodendrocytes for 11.5\%, and microglia for 5.2\%. The deviation matrix contained 30,058 positive and 29,989 negative entries, while the minimum total coefficient was exactly zero. It had two resolved singular values ($3.24\times10^{-5}$ and $8.96\times10^{-6}$), with the remaining three below $1.4\times10^{-10}$, supporting a low-rank but not rank-one deviation structure.

A matched ablation constrained the deviations themselves to be non-negative while retaining the gene-summed annotations, frozen context profile, folds, penalty grid, optimizer, and numerical precision. It also selected $\lambda=100$, with mean held-out MSE 13.9287. The signed model's 0.34\% lower MSE was not resolved across folds: the paired signed-minus-nonnegative difference was $-0.0468$ (SE $0.1051$; paired $t$-test $p=0.667$). The non-negative full fit was rank one and had objective 9.7025, whereas the signed rank-two fit had objective 9.2668. Thus signed deviations remove an unnecessary biological restriction and improve the full-data fit, but do not provide detectable held-out predictive improvement in this analysis.

We ranked SCZ genes by positive fitted deviation contribution, $\sum_c M_{gc}D_{gc}$, where $M_{gc}$ is the annotation mass linking gene $g$ to context $c$. The leading global genes included \textit{TRIM9}, \textit{ZBTB16}, \textit{KLC1}, \textit{RASA3}, \textit{KIF21B}, \textit{TSNARE1}, \textit{FMNL2}, \textit{HLA-A}, \textit{KCNB1}, and \textit{QKI}. Because the nuclear penalty induces low rank rather than elementwise sparsity, ordered g:Profiler analysis used the smallest leading set accounting for 25\% of positive deviation contribution, with a 50\% sensitivity analysis, against all 16,283 assayed genes as background. The 25\% sets contained 326 global genes and 196--245 context-specific genes. The neuronal ranking was enriched for synapse organization, Rett syndrome, and a potassium-channel complex; the microglial ranking was enriched for cytokine, interferon, and immune-receptor signaling. At the 50\% threshold (921 global and 613--716 context-specific genes), neuronal and global enrichments strengthened for synapse organization, neuronal-system, trans-synaptic-signaling, neurogenesis, and 16p11.2 deletion terms, while microglial immune enrichment persisted. Context-specific rankings partly reflect differences in annotation mass. These within-ranking g:SCS-adjusted results are therefore exploratory pathway hypotheses rather than independent discoveries.
