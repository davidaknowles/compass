#!/usr/bin/env python
"""Build BaselineLD-compatible annotations from lifted Glass brain ABC links."""
...
from compass.baselineld import read_bim, write_continuous_abc_annotations, write_hapmap3_sumstats
from compass.glass_abc import GLASS_ABC_V2_CELLS
...
def main() -> None:
    ...
    out_dir = (
        ...
        if args.out_dir
        else data_root / "results" / "official_sldsc_glass_abc_inputs"
    )
    ...
    columns = [f"ABC_glass_{cell}" for cell in GLASS_ABC_V2_CELLS]
    annotation_counts = write_continuous_abc_annotations(
        bims,
        abc_path,
        list(GLASS_ABC_V2_CELLS),
        out_dir / "annotation",
